...
suite("ConfigService", () => {
        ...
        feeds: [
          {
            name: "main",
            type: FeedType.JSONL,
            url: `http://bulk.openweathermap.org/snapshot/${process.env.OPENWEATHER_API_KEY}/weather_14.json.gz`,
            charset: "",
            key_column: "city.id",
  ...
  test("create a new spreadsheet, save and load data", async function () {
    ...
    try {
      const response = (
        await sheetsAPI.spreadsheets.create({
          requestBody: {
            sheets: [
              {
                properties: {
      ...
      let config: Config = {
        title:
          "TRGR:[Debug][Unit-tests]create a new spreadsheet, save and load data",
        execution: {
          advertiserId: "506732",
          campaignId: "3242703",
          //adjustBids: false,
