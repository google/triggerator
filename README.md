...
## Configuration
The only configuration the application supports is in environment variable (that can be specified only via `app.yaml`). If you need to change an environment variable you need to redeploy app.
...
* MAILER_CONFIG_PATH - 
* LOG_LEVEL - minimus log level ('info' or 'debug'), for production it's 'info' be default)
